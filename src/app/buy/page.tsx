...
import type { CartItem, ParcelCartItem, SingleCartItem, MetalCartItem } from "@/lib/types";
...
type AppTab = "parcels" | "singles" | "metals";
  ...
  return (
    <div className="min-h-screen bg-zinc-50 pb-28">
      ...
      <div className="max-w-lg mx-auto px-4 pt-4 space-y-4">
        ...
        {/* Active form */}
        {tab === "parcels" && (
          <ParcelsForm vendor={activeVendor} onAdd={(item) => addItem(item)} />
        )}
        {tab === "singles" && (
          <SinglesForm vendor={activeVendor} onAdd={(item) => addItem(item)} />
        )}
        {tab === "metals" && (
          <MetalsForm vendor={activeVendor} onAdd={(item) => addItem(item)} />
        )}
        ...
        {cart.length > 0 && (
          <div className="bg-white rounded-xl border border-zinc-200 p-4 space-y-3">
            <div className="flex justify-between items-center">
              ...
              <button type="button" onClick={() => setGroupedView((v) => !v)} className="text-xs text-zinc-500 underline">
                {groupedView ? "Individual" : "Grouped"}
              </button>
            </div>
