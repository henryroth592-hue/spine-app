...
export default function MeleeForm({ vendor, onAdd }: Props) {
  ...
  const pricePerCt = assortment.prices[sizeRange] ?? null;
  const weight = parseFloat(weightInput) || 0;
  const calcTotal = pricePerCt !== null && weight > 0 ? Math.round(pricePerCt * weight) : null;
  const lineTotal = totalOverride !== ""
    ? (parseInt(totalOverride.replace(/[^0-9-]/g, "")) || 0)
    : calcTotal;

  function handleAdd() {
    if (!vendor || pricePerCt === null || weight <= 0 || lineTotal === null) return;
    onAdd({
      id: uid(),
      itemType: "melee",
      vendor,
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-zinc-200 p-4 space-y-4">
        ...
        <button type="button" onClick={handleAdd}
          disabled={!vendor || pricePerCt === null || weight <= 0}
